...
export default function ContactPage({
  ...
  onSubmitFeedback?: (fb: ContactFeedback) => void;
}) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!fullName.trim() || !email.trim() || !message.trim()) {
      setError('Vui lòng điền đầy đủ Họ và tên, Email và Lời nhắn.');
      return;
    }
    setError('');
    onSubmitFeedback?.({
      ...
      <section className="mx-auto max-w-[1200px] px-4 pb-20 pt-12 sm:px-6 lg:px-8 lg:pb-24 lg:pt-16">
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-[1.15fr_0.85fr] lg:items-start">
          <div className="public-surface rounded-[18px] border p-6 shadow-[0_10px_30px_rgba(15,23,42,0.04)] sm:p-8">
            ...
            {submitted && (
              <p className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-[14px] font-medium text-emerald-700">
                Đã gửi yêu cầu thành công! Đội ngũ hỗ trợ sẽ phản hồi bạn sớm nhất.
              </p>
            )}
            {error && (
              <p className="mt-4 rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-[14px] font-medium text-rose-700">
                {error}
              </p>
            )}
