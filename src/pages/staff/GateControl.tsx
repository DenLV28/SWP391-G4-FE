...
import { createRfidScan, updateRfidScan, subscribeToRfidTaps, sendGateCommand, getLastEntryScan } from '../../services/rfidScanService';
...
export default function GateControl({
  ...
  const runRfidPipeline = async (
    ...
    const isExit = (opts?.direction ?? activeDirection) === 'exit';
    const [{ image, plate: ocrPlate }, lookupResult, entryScan] = await Promise.all([
      webcamActive ? captureAndOCR() : Promise.resolve({ image: '', plate: '' }),
      fetchRfidInfo(uid),
      isExit ? getLastEntryScan(uid) : Promise.resolve(null),
    ]);
    ...
    // Xe ra: hiển thị biển số lúc vào để nhân viên đối chiếu với camera
    const entryNote = isExit && entryScan?.licensePlate
      ? `\n↓ Biển số lúc vào: "${entryScan.licensePlate}" (${entryScan.createdAt})`
      : isExit && !entryScan
        ? '\n⚠️ Không tìm thấy hồ sơ vào bãi của thẻ này.'
        : '';
    setAutoPipelineNote(
      !scanRecord
        ? `${prefix}Không lưu được lượt quét vào hệ thống — kiểm tra kết nối backend.`
        : ocrPlate
          ? `${prefix}Camera hiện tại: "${ocrPlate}"${entryNote}`
          : webcamActive
            ? `${prefix}Đã lưu lượt quét, nhưng không đọc được biển số từ camera.${entryNote}`
            : `${prefix}Đã lưu lượt quét. Bật camera để tự động chụp & nhận diện biển số.${entryNote}`,
    );
  };
